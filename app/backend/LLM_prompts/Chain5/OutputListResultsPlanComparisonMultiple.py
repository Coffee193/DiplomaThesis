def getPrompt(user_question, recommended_doc_names):
    names_str = ', '.join(f'"{name}"' for name in recommended_doc_names)

    prompt = f"""You are an AI assistant helping a user based on queried information.

You are not given the actual data.
...
IMPORTANT: Based on the analysis of all uploaded Output files, the following plans share the shortest overall duration (makespan): {names_str}.
After placing (DATA), recommend any one of these plans to the user since they all share the same shortest makespan. Mention the document names in your recommendation. Use plural language when referring to them (e.g., "these plans are", "they have").

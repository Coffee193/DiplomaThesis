...
def ClassifyRootSearch(user_question):
   ...
   sys_prompt = f"""You are a classifier. Your task is to analyze a user's question and return EXACTLY ONE word from the predefined list below as well as your thinking process. 

Your output should be a single word (number) and nothing else. The word you pick should always match the one you ended up choosing based on your thinking process. Think before you pick. Never make assumptions.
You should only output a number from the ones listed below without anything else. Do not explain anything

...
llm_answer = ClassifyRootSearch(user_question)
print(llm_answer.message.content)
answer = llm_answer.message.content

### Check Output From Classifier on Chain 1 ###
answer = answer.replace('\n', '')
if('`' in answer):
   answer = answer.replace('`', '')
   if(answer[:4] == 'json'):
      answer = answer[4:]
   print('Answer Output Wrong Format ** Fixed It')
...
try:
    ...
    print('\n2\n')
    if('pick' not in answer or type(answer['pick']) != int or (answer['pick'] not in numbertokeyword)):
        print('\n3\n')
...
''' REDUNTANT
if( (answer.isdigit() == False) or (answer not in numbertokeyword)):
    prompt_no_understand = f"""You were given the following question: {user_question}. For some reason you couldn't process it.
    Inform the user to write it more clearly"""
    print(chat('llama3.1', messages = [{'role': 'user', 'content': prompt_no_understand}]).message.content)
'''

search = numbertokeyword[answer['pick']]
...
answer = chat('llama3.1', messages = [{'role': 'user', 'content': prompt}])
...
### EXTRACTION PART ###
try:
    retrieve_info = json.loads(answer.message.content)
    # retrieve_info = IDValueToCorrectForm(retrieve_info)
    # retrieve_info = NameValueToCorrectForm(retrieve_info)
except:
    retrieve_info = None
...
if retrieve_info == None:
    '''
    prompt = f"""You are an AI assistant tasked with analyzing user input according to a set of instructions. However, during processing, an unexpected issue occurred that prevented you from fully understanding or completing the analysis of the user’s request.

Politely inform the user that you were unable to proceed due to ambiguity or lack of clarity in their input. Ask them to rewrite their question in a clearer, more precise, and detailed way so that you can better assist them.

Your response should:

- Be polite and professional
- Avoid blaming the user directly
- Clearly state that the request could not be processed as-is
- Encourage the user to provide more specific details or rephrase their question

Example tone:
“I'm sorry, but I wasn't able to fully understand your request due to some ambiguity. Could you please rephrase or clarify your question with more details so I can assist you more effectively?”
...
"""
    ...
    print(chat('llama3.1', messages = [{'role': 'user', 'content': prompt}]))
    uijHSDUIJSDHJ
else:
    if(retrieve_info['attribute'] == True):
        
        if(search == 'resource' or search == 'tasks'):
